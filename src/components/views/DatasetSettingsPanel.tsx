...
import { CalendarRange, MapPinned, Settings2, SlidersHorizontal, UserRoundSearch } from 'lucide-react';
...
export function DatasetSettingsPanel({ dataset }: { dataset: Dataset }) {
  ...
  function patch(key: keyof ViewConfig, value: string | undefined) {
    updateViewConfig(dataset.id, { [key]: value });
  }
  ...
  return <div className="mx-auto max-w-5xl space-y-5 pb-8">
    <section className="rounded-xl border border-slate-200 bg-white p-5">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <FieldSelect label="订单状态字段" value={config.statusFieldKey} fields={categorical} onChange={value => {
      ...
      {config.statusGroups?.length ? <div className="mt-4 rounded-lg bg-slate-50 p-3">
        ...
        <div className="flex flex-wrap gap-2">{config.statusGroups.map(group =>
    ...
    <section className="rounded-xl border border-slate-200 bg-white p-5">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <FieldSelect label="大区" value={config.geoRegionKey} fields={categorical} onChange={value => patch('geoRegionKey', value)} />
        <FieldSelect label="省份" value={config.geoProvinceKey} fields={categorical} onChange={value => patch('geoProvinceKey', value)} />
        <FieldSelect label="城市" value={config.geoCityKey} fields={categorical} onChange={value => patch('geoCityKey', value)} />
      </div>
    </section>
    ...
    {editingStatus && config.statusFieldKey && <StatusGroupEditor
      allValues={uniqueValues(dataset, config.statusFieldKey)}
      groups={config.statusGroups ?? []}
      onSave={groups => updateViewConfig(dataset.id, { statusGroups: groups })}
