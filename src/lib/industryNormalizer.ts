...
export function normalizeIndustry(value: unknown): string {
  ...
  if (!text || /^(?:\(跳过\)|（跳过）|跳过)$/.test(text)) return '';
  ...
  if (/水利|环境和公共设施|环境与公共设施/.test(text)) return '能源矿产与公用事业';
  if (/政府|事业单位|国企|国有企业|国有\/集体企业|检察院|军队|部队|军人|军警|社区|社会组织|社会团体|香港政府|退役军人|国际组织/.test(text)) return '公共管理与事业单位';
  ...
  if (/制造|生产|加工|工程师|技术员|研发人员|操作人员|操作工|汽车|汽配|纺织|羽绒|服装|家具|家电|食品工厂|维修|汽修|机动车检测|弱电工|技术工|工人|飞机维修|建材|五金|试驾车/.test(text)) return '制造与工业服务';
  return '其他';
}

export function normalizeIndustryValues(values: unknown[]): string {
  const texts = values.map(value => String(value ?? '').trim())
    .filter(value => value && !/^(?:\(跳过\)|（跳过）|跳过)$/.test(value));
  if (!texts.length) return '';
  if (texts.every(value => /^(?:其他[〖【])?(?:无|无业|待业|没有工作|不便透露|秘密|其他)[〗】]?$/.test(value))) return '';
  const primaryRaw = texts[0];
  const primary = normalizeIndustry(primaryRaw);
  const needsSupport = new Set(['其他', '自由职业（行业未明确）', '未就业/学生']);
  const supportText = texts.slice(1).join(' ')
    .replace(/自由职业者\s*[（(][^）)]*[）)]/g, '')
    .replace(/其他[〖【](?:无|无业|待业|不便透露|秘密|其他|自由职业)[〗】]/g, '');
  const support = normalizeIndustry(supportText);
  if (/^其他[〖【]/.test(primaryRaw) && support === '公共管理与事业单位') return support;
  if (primary && !needsSupport.has(primary)) return primary;
  if (support && !needsSupport.has(support)) return support;
  return /没有工作|主妇|主夫|学生|无业|待业/.test(primaryRaw) ? '' : '其他';
}
...
export function normalizeIndustryFields(
  ...
  const industryKeys = keys.filter(key => /从事.*行业/.test(key));
  if (!industryKeys.length) return records;
  const supportKeys = keys.filter(key => /工作单位类型|岗位级别|职业类型/.test(key));

  return records.map(record => {
    const normalized = { ...record };
    for (const key of industryKeys) {
      normalized[key] = normalizeIndustryValues([record[key], ...supportKeys.map(support => record[support])]);
    }
    return normalized;
  });
}
