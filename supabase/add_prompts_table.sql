-- 可重复执行：每次运行只会更新/新增 prompt 数据，不会报错
-- AI Prompt 配置表（已存在则跳过）
...
begin
  if not exists (
    ...
    execute 'create policy "service role full access" on ai_prompts using (true) with check (true)';
  end if;
end$$;
